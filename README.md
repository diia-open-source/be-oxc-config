...
This repository contains the package with shared node.js OXC toolchain configurations.
...
```
git clone https://github.com/diia-open-source/be-oxc-config.git oxc-config
```
...
In order to install and use the linked dependencies for `oxc-config` the following command can be used:
